...
# TODO FIGURE THIS OUT (required)            df = m.create_relational_field(f.name, f.type, rel_model_id, rel_name_id, f.required)
    ...
    def create_url_for_form_receiver(self, uie):
        ...
        data_string = ' '.join(['{{ Page.%s }}.id' % e.name for e in uie.get_needed_page_entities]) # result should be something like "{{ Page.Book }}.id {{ Page.Class }}.id"
        if data_string != '':
...
## END HACKING
    ...
    def create_form_receiver_for_form_object(self, uie):
        ...
        fr = DjangoCustomFormReceiver(fr_id, uie._django_form.identifier)
        args = []
        for e in uie.get_needed_page_entities():
            model_id = e._django_model.identifier
    ...
    def add_relation_assignments_to_form_receiver(self, uie):
        translate = lambda s: self.v1_translator.v1script_to_app_component(s, uie._django_form_receiver, py=True)  # it's going to utilize args and locals from here.
        form_model = uie.container_info.form # bind to this name to save me some typing
        if form_model.action not in ['create', 'edit']:
            return None
        form_obj = uie._django_form_receiver
        for r in form_model.actions:
            form_obj.relation_assignments.append((translate(r.set_fk), translate(r.to_fk)))
    ...
    def add_saving_of_related_assignments(self, uie):
        ...
        for l, r in form_model.get_actions_as_tuples():
            future_l_identifier = translate(self.string_ref_to_inst_only(l))
            if l.startswith('this'):
                before_save_saves.add(future_l_identifier)
            else:
                after_save_saves.add(future_l_identifier)

        form_obj.before_save_saves = before_save_saves
        form_obj.after_save_saves = after_save_saves
