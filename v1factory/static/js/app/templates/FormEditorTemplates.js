...
FormEditorTemplates.template = [
  '<div class="fields-panel panel">',
    ...
    '<% _(entity.get("fields").models).each(function(field) { %>',
      '<label><input type="checkbox" id="field-<%= field.cid %>" class="field-name-box" value="<%= field.get(\'name\') %>" <% console.log(form.get(\'fields\').filter(function(d){ return d.get(\'name\') == field.get(\'name\')})); if(form.get(\'fields\').filter(function(d){ return d.get(\'name\') == field.get(\'name\')}).length > 0) { %> checked <% }; %>><%= field.get(\'name\') %></label>',
    '<% }); %>',
